...
stakeInput.value = "1";
callBtn.parentElement = putBtn.parentElement = stakeInput.parentElement = { contains: () => false };
...
const allEls = [callBtn, putBtn, stakeInput];
sandbox.document = {
...
const chromeStub = {
  ...
  storage: {
    local: {
      ...
      set: (obj) => { Object.assign(storageMap, obj); },
    },
...
const placed = Q.placeTradeDom({ dir: "CALL", amount: 5 });
...
async function main() {
  ...
  // next bar → allowed (advance clock past the 1s min-interval)
  advance(6000);
  const start2 = decisions.length;
  await controller.handleSignal(makeSignal("PUT", barT + 60000));
  ...
  await controller.handleSignal(makeSignal("CALL", barT + 120000));
  ...
  await controller.handleSignal(makeSignal("CALL", barT + 300000));
  await controller.handleSignal(makeSignal("PUT", barT + 300001));
  ...
  await controller.handleSignal(makeSignal("CALL", barT + 300000)); // same bar+dir as start4's CALL
